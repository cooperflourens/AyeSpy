...
class Scraper:
    def __init__(self, db, user, password, api_key, min_congress, max_congress,
        ...
        self.headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux i686; rv:110.0) Gecko/20100101 Firefox/110.0.'}
    ...
    def scrape(self):
        ...
        results_page = 'https://api.govinfo.gov/collections/CHRG/1776-01-28T20%3A18%3A10Z?offset=0&pageSize=100&/congress={1}&api_key={0}'
        for congress in self.congresses:
            page = results_page.format(self.api_key, congress)
            while True:
                request = Request(page, headers=self.headers)
                print("GOT HERE")
                hearings_list = json.loads(urlopen(request).read())
                print("Got Here")
    ...
    def _save_data(self, hearing_json):
        ...
        try:
            transcript_request = Request(htm_page.format(self.api_key, hearing_id), headers=self.headers)
            transcript = urlopen(transcript_request).read()
            transcript = re.sub('\x00', '', transcript.decode('utf8'))
            
        except HTTPError:
            transcript = ''

        meta_request = Request(mods_page.format(self.api_key, hearing_id), headers=self.headers) 
        meta = urlopen(meta_request).read()
